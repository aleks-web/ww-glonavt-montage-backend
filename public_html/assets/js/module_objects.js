...
// Загрузка таба документов объекта
...
// Подгрузка модалки - новый документ для объекта
function load_fmodal_new_objdoc(object_id) {
    $region_wrapper = $('#fmodal-new-objdoc-wrapper');
    let url = API_V1_URLS.objects.render + 'fmodal-new-objdoc'; // API_V1_URLS - Смотрим в main.js
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status == "success") {
                ...
                $.fancybox.open({
                    src: '#fmodal-new-objdoc',
                    type: 'inline'
                });

                dd(response, `Render fmodal-new-objdoc.twig ${url}`);
            }
